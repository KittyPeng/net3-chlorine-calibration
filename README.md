...
`baseline.npz`, the 8192-candidate prediction library, is **not** in this repository. It is
118 MB, past GitHub's per-file limit, and twenty-three scripts read it, including `paper_figs.py`.
Step 1 regenerates it, and `cache_manifest.json` records its SHA-256 as `baseline_npz_sha256`, so
a regenerated copy can be checked against the one every published result was computed from:

```bash
shasum -a 256 Net3/baseline_cache/baseline.npz
```

The `.npy` prediction caches for the displaced-prior and bulk-decay arms are absent for the same
reason and are regenerated by their own steps. Everything in `Verify` above runs without any of
them; only re-plotting the figures and re-running the candidate-level calculations needs them.
...
| `Net3/appendix_tables.py`    | the twenty-four appendix tables                                        |
